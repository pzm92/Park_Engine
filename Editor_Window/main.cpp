...
// #pragma comment (lib, "..\\Debug\\ParkEngine_Window.lib")
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance, // 프로그램의 인스턴스 핸들
                     _In_opt_ HINSTANCE hPrevInstance, // 바로 앞에 실행된 현재 프로그램의 인스턴스 핸들, 없을 경우에는 NULL
                                                        // 지금은 신경쓰지 않아도 되는 값이다.
                     _In_ LPWSTR    lpCmdLine, // 명령행 으로 입력된 프로그램 인수라고 하는데 거의 쓸 일이 없다.
                                                // 참고 : 비주얼 스튜디오 명령 인수 <-- 구글 검색
                     _In_ int       nCmdShow) // 프로그램이 실행될 형태이며, 보통 모양정보등이 전달된다.
{
    ...
    // 전역 문자열을 초기화합니다.
    LoadStringW(hInstance, IDS_APP_TITLE, szTitle, MAX_LOADSTRING); // 문자열을 불러오겠다.
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    // wcex.hbrBackground = (HBRUSH)(GetStockObject(5));
